...
#: 900s of 1s-resolution rows is the worst case for one series; also caps the
...
async def _series(session, model, point_cls, *, device_id: uuid.UUID, since: datetime) -> list:
    rows = (
...
async def recent_samples(
    ...
    return RecentSamplesOut(
        ...
        disk_io=await _series(session, DiskIoSample, DiskIoPoint, device_id=device_id, since=since),
        net=await _series(session, NetSample, NetPoint, device_id=device_id, since=since),
        latency=await _series(
            session, LatencySample, LatencyPoint, device_id=device_id, since=since
        ),
