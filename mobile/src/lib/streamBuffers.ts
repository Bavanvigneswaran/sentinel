 ...
 *
 * Copied from web/src/lib/streamBuffers.ts. The only change is the capacity:
 * 15 minutes of 1s samples is more than a phone-sized chart can usefully
 * show, and four buffers of 900 points each is memory a background-able app
...
// 5 minutes at 1s live resolution. Matches PRIME_WINDOW_SECONDS in
// hooks/useDeviceStream.ts, so the primer fills the buffer exactly once and
// the visible window is the same whether you just opened the screen or have
