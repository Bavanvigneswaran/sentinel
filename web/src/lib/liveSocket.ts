/**
 * The viewer WebSocket client.
 *
 * One shared connection for the whole tab, module-scoped the same way
 * lib/api.ts single-flights /auth/refresh: every subscriber shares it rather
 * than opening a socket per component. subscribeDevice() ref-counts, so two
 ...
 * agent/sentinel_agent/transport/client.py — the same reasoning applies:
 * without jitter, every tab that lost its connection to the same backend
...
class LiveSocket {
  ...
  private async connect(): Promise<void> {
    ...
    try {
      ...
      // from a previous attempt was already spent or has expired.
      const { ticket } = await apiFetch<TicketResponse>("/ws/tickets", { method: "POST" })
      const ws = new WebSocket(`${viewerWsUrl()}?ticket=${encodeURIComponent(ticket)}`)
  ...
  private scheduleReconnect(): void {
    // Nothing to watch: let the socket stay closed until the next
    // subscribeDevice() call reopens it.
